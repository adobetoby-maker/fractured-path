...
# AUTHOR REPORT — v3-runs/book-07/reports/ch19-author-r1.json

{
  ...
  "deviations": [
    "First pass came in at 5,523 words and was trimmed inside the band by a line pass on catalogue and connective prose only (no beat, obligation, or mandated line removed). Final 5277.",
    "Two mandated lines (Oryn's diagnosis; Teague's closing sentence) were first split by attribution and made contiguous.",
    "Oryn's unnamed remark 'Ask first' before the ascent is an invention in her register (the rule said aloud on the floor), not a reading.",
    "The one bowman Cael meets at contact is put off the rim's lip 'on his feet' and not pursued — no death, no capture, per the packet; reported so the editor can judge the removal beat as 'X removed Y's ability to Z' (Cael removed the rim's ability to hold the bowl by unbinding its only controlled exit and attacking from the side its design said was impossible)."
  ],
  "blockers": []
}
