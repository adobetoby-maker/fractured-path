...

{
  ...
  "revisions": {
    ...
    "arc": "arc-b7-v2 (books/book-07-void-roads/CHAPTER_ARCHITECTURE.md, editor PASS r4, Chapter 11 card)",
    "state": "state-b7-post-ch10 (v3-runs/book-07/STATE_RUNNING.md + base snapshot)",
  ...
  "pov": {
    ...
    "knowledge_boundary": [
      "Cael is eighteen; nine + anomaly. Party SIX. Inside the Fallow Ring at dusk, ~140 m in, the light going. Known from Chapter 10: perfect circle, 200 m radius, sharp line; every Path dies inside; Oryn's hands don't work inside; the den is under the far lip; stillhounds hunt declarations (Ch3) and den here because prey can't declare (Ch10, Seln). Cael alone knows his fragments work inside; NOBODY else knows until they SEE it in this chapter.",
      "The winning idea is Chapter 3's, inverted and taught there: the only declaration on this floor is Cael; stillhounds follow the loudest declaration; five practitioners get their Paths back at the stride outside the line; Karis's ignition-wall (Ch6) is a thing the hounds have never met. No other mechanism resolves the fight.",
      "Shadow-adjacent may be used ONCE, inside, to reach Karis unseen by the pack, witnessed only by the circle and Oryn (no outside witness) — the seal's logic holds. Every other fragment may be deployed in the run to the line. Wind-adjacent, full and loud, is the declaration the pack follows.",
      "Lira's ankle goes a THIRD time at the lip's edge in the first exchange (ford, ledge, lip). A hound bites through her forearm and does not let go; Brom's bolt kills it at four meters; she cannot burst it off. Oryn's Tide fails on the wound INSIDE (the bible's beat, staged at the worst moment); she binds with cloth and her knees: 'Move. I can't fix this here. Move.' OUTSIDE the line her Path returns at the stride and she mends the arm at the line by lamplight for an hour — the arm is SAVED, vessels and deep tear closed, and a mend of that depth bears no load for a season: Lira is one-armed by Oryn's order from here to first snow.",
      "Costs at close: Karis's throat bruised, voice gone; Brom's bolt-hand cut open on the crossbow string; Seln unmarked; Cael spent to the ground (first full-suite deployment since the Daeva match, on ribs three weeks healed), calf reopened on the lip; Oryn drained by the mending.",
      "Nobody theorizes why Cael's fragments work; nobody says 'Quieting'; the ring's floor is not damaged or explained."
    ]
  },
  "purpose": "The fight inside the Quiet: the series' first choreography built on a boundary as terrain — a retreat with a geometry toward a line that turns Paths back on, past animals that live inside it because prey can't fight there — resolved by the ford's tactic inverted: Cael becomes the only declaration on the floor and leads the pack OUT across the line into five practitioners who just got their Paths back. Everyone sees what he did inside. Nobody says anything, because the priority is Lira's arm. Oryn's hands fail at the worst possible moment and return at the stride.",
  "scene_shape": {
    "opening_state": "Dusk, 140 m inside, the six of them at the ring; nine hounds come silent from the den under the far lip.",
    "pov_goal": "Get six people to a line two hundred meters from anywhere, alive, on ground where only he works — without, until he must, using the one thing that will save them and end his silence.",
    "opposition": "Nine stillhounds on their own floor; the cut-dog geometry; a healer with no hands; a line sixty, then forty, then twenty meters too far.",
    "turn": "FIRST: the cut — the dog that takes the road is between them and the perimeter; Seln, Pathless, calls the lane that isn't cut. SECOND: Brom takes the rear with a carter's crossbow and his body — a big man with good footwork and a bolt holding a lip of fused stone; the pack hunts by sound now and the unit is quiet by four books of training. THIRD: Lira — ankle going a third time at the lip — runs the lip with a knife; a hound takes her forearm and holds; Brom's bolt through its eye at four meters; Oryn on her in a breath, both hands, the reading returning nothing in her hands, blood to the wrist, her Path a stride and a half of stone away: cloth and knees, 'Move. I can't fix this here. Move.' FOURTH — the turn: sixty meters from the line, Karis goes down under a hound, hands on its throat, no lattice; the pack converges on the noise.",
    "choice": "Cael stops holding. Wind-adjacent, full, loud — the loudest thing in the Quiet — and he goes PAST the pack toward the line, the only declaration on the floor, moving, and every hound turns and follows the thing it hunts by, exactly as the ford taught, because they have never met prey inside the circle that could be loud. (Shadow-adjacent once, to reach Karis unseen first, if the choreography needs it.)",
    "outcome": "FIFTH: he crosses the line at a run with nine hounds behind him and the unit already over — Lira dragged, Karis carried by Brom; the pack comes across the stone after the declaration into a place where its den advantage is a stride behind it and five practitioners have their Paths back at the stride; Karis, on her knees, voiceless, lights the lattice-line across the line's inside edge — a wall of ignition on the one meter of ground where the hounds have never needed to be careful. They break out through the scrub, not back into the ring. Two down to Brom's bolts; seven gone. Win: Cael took away the pack's ability to fight where its prey cannot, by being the one thing on the floor it could follow out. Costs ledgered outside the line; Oryn's Path back at the stride; the hour's mending at the line by lamplight; the arm saved and load-restricted for a season; Cael's Shadow use witnessed by the circle and Oryn only; everyone saw the seventh thing on the floor work and nobody says anything because the priority was Lira's arm.",
    "closing_state": "The chapter ENDS ON DIALOGUE (the previous chapter ended on the log): Lira, on the road, the arm saved and slung and the ankle not, looking at the ring behind them and not at Cael: 'It worked in there.' A beat. 'Yours.' Cael: 'Yes.' Lira: 'Later.' Cael: 'Later.'"
  },
  "obligations": {
    "must_include": [
      "Five exchanges as staged, with the boundary as terrain: the lip, the den side, the lane, the line as objective; geography drawable at every pause; at least two beats caused by terrain (the lip of fused stone Brom holds; the line itself; the one meter inside the edge where Karis lights).",
      "Stillhounds hunt by declaration and by sound; silent bodies nearly invisible to them; the unit quiet by training; the pack has never met prey inside that could be loud (all taught earlier; may be restated in Cael's cataloguing).",
      "Lira's third ankle turn at the lip; the forearm bite; Brom's four-meter bolt; Oryn's failed reading inside with her two-sentence order verbatim; the binding with cloth and knees.",
      "Karis down under a hound at sixty meters; Cael's Wind-adjacent declaration and the run PAST the pack to the line; the pack following the declaration out; five Paths returning at the stride; Karis's ignition-wall on the inside edge; the break; two dead, seven gone.",
      "Costs outside the line as listed; Oryn's Path returning at the stride and the hour's mending at the line by lamplight; the arm's state DEFINED on the page (saved; vessels and deep tear closed; bears no load for a season; Oryn's order: not to hold, strike, or catch until she says; one-armed to first snow).",
      "Cael's deployment inventory honest: Wind-adjacent full; Shadow-adjacent once inside (witnessed by circle and Oryn only); other fragments as the run requires; spent to the ground; first full-suite use since the Daeva match; calf reopened.",
      "Everyone saw; nobody says anything; the priority was Lira's arm.",
      "The chapter ENDS on the Lira/Cael exchange verbatim.",
      "Manuscript format: H1 '# Chapter 11 — The Quiet', prose only, '---' breaks, ends on the last line of prose."
    ],
    "plants": [
      "Cael's fragments work inside the Quiet — now SEEN by five people (Ch12 silence; Ch16 disclosure).",
      "Lira one-armed for the season (Ch14–24).",
      "Oryn's Path returns at the stride (Ch19, Ch21).",
      "'Later.' — Lira first (Ch16)."
    ],
    "payoffs": [
      "Ch3's bait-with-one-declaration, inverted.",
      "Ch6's ignition-wall.",
      "Ch10's den, sun, and Oryn's hands."
    ],
    "prohibited_outcomes": [
      "Nobody dies; nobody loses a limb; no injuries beyond those listed; no wolves/other fauna; the hounds do not speak, think, or carry a Path.",
      "No fragment other than those listed resolves anything; no unexplained effect on the pack; the floor is not damaged.",
      "No theory of why Cael's fragments work; nobody says 'Quieting'; no survey stake beat (Ch12).",
      "No outside witnesses; no Teague; no letters; no Vastin.",
  ...
  "invention_budget": {
    "allowed": [
      "Full choreography texture (dusk light on grey stone, the hounds' silence and grey, footing on the lip, the sound of a crossbow string, blood on stone, the line felt in the body when crossed); the mending by lamplight as sensation and cost; each companion's fight-style in character (Lira stripped to bone; Brom weight and bolt; Karis hands on a throat; Seln position; Oryn the healer reaching a patient).",
      "Dialogue in established voices; Seln's calls in one flat sentence each.",
      "Cael's cataloguing under pressure (claim/evidence/ruling in fragments)."
    ],
    "approval_required": [
      "Any new named entity; any hound behavior beyond declaration/sound hunting, den defense, and animal adaptation; any new fragment mechanic; any change to the injury list."
    ],
    "forbidden": [
      "Deaths; unexplained mechanisms; theory; disclosure; outside witnesses; a log or image ending; metadata."
    ]
  },
  "context_files": [
    {
    ...
    {
      "kind": "previous_scene",
      "label": "Chapter 10 as drafted — the immediate seam (the den, the log)",
      "path": "v3-runs/book-07/drafts/ch10.md",
      "required": true
    },
    {
      "kind": "reference",
  ...
  "exceptions": [],
  "output": {
    "draft_path": "v3-runs/book-07/drafts/ch11.md",
    "report_path": "v3-runs/book-07/reports/ch11-author-r1.json",
    "editor_report_path": "v3-runs/book-07/reports/ch11-editor-r1.json",
    "verifier_report_path": "v3-runs/book-07/reports/ch11-verifier-r1.json",
    "target_words": 4600,
...
# CONTEXT — Chapter 10 as drafted — the immediate seam (the den, the log) (v3-runs/book-07/drafts/ch10.md)

# Chapter 10 — Two Hundred Meters

Karis had the cord out before the light was on the road.

He catalogued the camp as he sat up, because it was the first morning he had ever woken beside a line. The stone in its rut, two fists of it, dew on the top and none underneath. The three boxes on the Thornwater side of the stone, one, two, three, as Brom had set them down. The mule, hipshot, cropping the verge with its rope round a scrub root. Seln, sitting where he had sat down the evening before, facing the way they had come, who might have slept and might not. Lira with the ankle's tape already done. Oryn on her back on the road with her coat over her and her hands folded on the coat, awake, looking at the sky. And Karis at the stone with the cord in both hands, running it through her fingers knot by knot, as a woman counts money she has counted already.

"Twenty meters," she said, to him, because he was the one looking. "Knotted at the fives. I checked it against the rule at Lowmarch." She looked past the stone at the ridge and the bend, and at the bowl beyond, where the ring stood on its floor with no sun on it yet. In the morning the floor was only grey. "Pike said we'd know. We know. Now I want to know where."

That was what she was for. He had watched her be it on a bench above a gorge, and at a barn table with three boxes open, and he had never seen it aimed at anything this size.

The procedure, said once at the stone, in her method register, while Brom ate.

Lira inside. One stride inside and no more, walking the line's inner edge with her Arbiter gone; if it came back she had strayed out, and she would say *back*, and step in, and say *gone*. Karis on the line, a stride outside Lira, with her own sigil present, laying the cord end over end along the ground and counting cords, and strides within the cords, because she had a number for her stride she trusted. Brom outside Karis, marking: a stone set down at the end of every cord, or a scrub stem broken and turned, whatever the ground gave. Seln ahead, because the ground had not been walked. Oryn at the stone with the mule, because a mule could not be told to keep to a stride. Cael at Karis's shoulder with the book open, writing the counts as she said them, because his hand was the faster and hers would be full.

"And what am I in this," Lira said. "Exactly."

"The instrument."

Lira looked at the ridge, and at the ring under it, and at her own two boots, and nodded once, and went to the stone.

---

She stepped over it at the rut, one stride, and said, "Gone," in the voice she used for a distance.

They went left off the road, along the shoulder's foot, into scrub that came to the knee. Karis laid the first cord from the stone with its knots along the ground and said "One," and Brom set a stone at its end, and Cael wrote *one*, and she took up the cord and laid it again. It was slow. It was meant to be. A cord, a count, a stone; a cord, a count, a stone; and a stride inside them Lira walking with her hand a little out from her side, not raised, as a woman walks a plank she can see.

"Back," Lira said, at the fourth cord, and stopped, and stepped in, and said, "Gone."

Karis did not stop. She had her own sigil for the other edge, and Cael watched it work on her face: a small check every few strides, the eyes going to the place and coming away, the way a man crossing a river checks the far bank. Twice in the first ten cords she said "Gone," quietly, of herself, and moved a half-stride out, and said "Back," and went on. Between Lira's stride and hers the line lay somewhere, and it never once lay wider than the two of them.

The scrub at the shoulder's foot gave way to grass, and the grass to a stony run where the ridge's rubble had come down, and then to scrub again, and through all of it the line bent left. Not sharply. The same small amount every cord, so that Karis, laying the cord straight, found its far end a hand outside Lira's stride every time and brought it in, a hand, every time, and by the twentieth cord had stopped saying anything about it and was simply laying the cord with the bend already in her wrist.

He wrote the counts and catalogued the country.

Inside Lira's stride: scrub, grass, stone, no different from the scrub and grass and stone outside it. No wall. No ditch. No change in the green. The ring stood on its floor in the bowl's middle, further off on this side than it had been from the road, and then nearer as they came round the bowl's east side. Nothing on the ground said *here*. If Lira had not been walking it with her hand out he could not have found the line with both eyes and a day.

At the third cord past the road — sixty meters, Brom bending with a stone in his hand — Brom straightened without setting it down.

"There's iron here."

Everybody stopped. He had his boot on it: a bar the thickness of a thumb standing a hand's height out of the scrub's root-tangle, brown, with the bush grown round it so that a man would have had to be putting a stone down on that exact place to feel it.

"Mark it," Karis said. "Keep the count."

Brom set his stone beside it, and they went on, and Cael wrote *three; iron*, and the line bent left.

At the far side of the bowl, where the ring's floor was nearest to the line, Seln was waiting on a rise of ground with his back to them and his face to the ring, and he did not turn as they passed him, and fell in behind. Cael counted the uprights from there because it was the closest he had been. Eleven. He counted them again at the next cord from a different angle, because one count is a count. Eleven.

At the sixty-second cord Lira said, "Road," and there was the road under her boots, and the stone in its rut, and Oryn at it with the mule, sitting on the middle box, and the sun well past noon. Karis laid the sixty-second cord and the stone was not at its end. She laid the cord's remainder along the rut and walked it in strides, counting aloud, and stopped with her heel at the stone.

"Sixty-two cords and twenty strides," she said. "Twelve hundred and fifty-six meters, and a piece."

She stood a moment, looking at the stone, and then took the book from him, and wrote, and while she wrote she talked, not to them.

"It bent left the same amount every cord. Every one. I never had to take the bend out of my wrist and I never had to put more in. A shape that does that is a circle." The pen went on. "A circle whose edge is two hundred meters from its middle goes round in twelve hundred and fifty-seven, near enough. I make it twelve hundred and fifty-six and a piece, with a stride's slop in the piece." She looked up, once, at the ring in the bowl's middle. "So from this stone to the middle of that ring is ten cords. Not ten and a piece. Ten. I'll walk it going in." Then, as a fact about a shape and nothing else: "A circle is a made shape."

She did not follow it. She wrote the count, and closed the book, and gave it back.

---

The iron first, because it was sixty meters off and Seln had already gone to it.

He was on one knee beside it when they came, with Brom's stone moved aside and the scrub's roots broken back from the bar with his knife, and he did not look up. What stood out of the ground was a forearm's length of square iron, driven, the top struck flat by whatever had driven it. Below the scale of five winters the metal was sound; where his thumb had rubbed the scale off the top there were marks struck into it with punches, and Cael, who had been reading punched marks off the corners of registry paper since he was fourteen, read them upside down before Seln said them.

"Registry stamp," Seln said. "And a number. Thirty-one."

The stamp was the one on the corner of every printed sheet in Karis's middle box. He did not need to look. He had stood under it on the wall of every office he had ever stood in.

"Survey stake." Seln turned it with his thumb, as far as a driven stake turns, which is not at all. "You drive them where you've measured to. Not where you've measured from." He put the thumb against the scale and pressed and looked at what came off. "Five years in the weather. A winter either side."

Karis ran the cord from the stake's foot to the place Lira's stride had passed it. It did not take a knot. It did not take a stride. The iron stood on the line to the width of a boot.

Nobody said who. Karis wrote the number down and the word on the stamp and the estimate and the width of Lira's boot, in that order, and Cael wrote them in his own book after her, and put a line under *thirty-one*, because a thing with a number on it has been entered somewhere, and a stake has a list.

The contract in his satchel said *surveyor's instruments abandoned at a site*. This was not an instrument. It was the thing instruments left.

---

The trials at the stone, after they had eaten, with the sun going over toward the ridge and the road.

Seln's rule first, said once: inside only. If a declaration rendered on the far side of the stone they would know something, and if it did not they would know something else, and only the first would call the dogs. Nobody declared on the near side. Nobody had needed telling, but he told them.

Lira first. She stood at the rut with her toes against the stone, on the near side, and set as she set for a burst: the knees, the breath taken and held, the whole of her put into the stride before the stride. Then she stepped over.

Cael watched the air. There was nothing to watch. He watched her instead, and saw her go through the burst from the inside — the shape of it in the legs and the back and the shoulders, the release, all of it, correct, the thing he had studied on a training floor at fourteen until he could see it with his eyes shut — and at the end of it she had taken one ordinary step and was standing one stride inside the line with the breath she had held coming out of her.

"That's a step," she said. "That was the whole of it. I did everything. It's in me. It's not in the air." She tried it again, small, a foot's worth. A foot's worth of step. "Nothing."

"Out," said Karis.

She stepped out backward, and Cael saw the thing arrive in her, at the stride, as a thing arrives: the eyes going to the place and finding it there.

"Back." She stood a moment with her hand half-lifted, as if to check the sigil against a light. "It doesn't come on like a lamp. It's just there, and it's where it was, like a clock that stopped and started again without losing the time."

Karis wrote that down verbatim, and asked her to say it again, and she said it again, and it went down the same.

Brom, then. He stepped over. He declared. Cael, who had watched Brom's Iron Skin come up in a hundred yards and rooms and once across a river, watched for the stillness that came into him when it came up, and it did not come. Brom looked at his own forearm and turned it, the raised dark line from wrist to elbow that Oryn's hands had left, and held it out, and Lira, inside with him, rapped it with her knuckles as she would have rapped a table.

"Skin," Brom said.

"Skin," Lira said.

He stepped out and stood with his shoulders doing the thing they had done on the road, once, the door closing the other way, and said nothing else, because Brom did not describe a door twice.

Karis went in without being asked, and stood a stride inside with the book under her arm and her scarred right hand out in front of her, and Cael saw her see it: the whole lattice, the way she saw it before she lit one, the points laid out on the air in the order her hand would take them.

"Here," she said, and moved the hand a foot. "Here. And there, on that stone, and the fourth above it." She held the hand where the first point went. Nothing went into it. "I can see every point. I know the order. I can't put anything in them." She lowered the hand, and looked at it, and then said the thing he had not thought of and she had. "It costs nothing. There's nothing to spend." She stepped out. "Back."

Seln stepped over the stone as he stepped over anything, and stopped, and stood, and for the space of three breaths looked at nothing in particular. Then he walked in. Twenty paces, through the scrub at the shoulder's foot, and Cael, who was watching for it, who knew what the man's Path did and had a piece of it, lost him anyway for the length of a breath between two bushes that would not have hidden a dog, and found him again coming back, and could not have said how either had happened.

"Gone at the stride," Seln said, at the stone. "Back at the stride." He looked at Karis's pen. "I've a knife and fifteen years. It was never most of what I do."

He said it as a man says his own weight. Karis wrote it down as one.

---

Oryn had not moved from the middle box through any of it. When Karis looked at her she stood up.

"Brom," she said. "The arm."

He gave it to her. She took it in both hands, outside the line, on the road, the forearm between her palms with the dark line under her thumbs, and her eyes went to nothing, and Cael counted with her, because he knew the count and it went to ten.

"Iron Skin," she said at ten. "Dense. The mend's holding. Same arm it was." She had had her hands on that arm on a mountain and at a table, and the count had come up the same each time. She did not let go. "Walk," she said to Brom. "One stride. I'll come with you."

They stepped over the stone together, her hands on his arm, and Cael watched her face because there was nothing else to watch, and saw the count stop in it. Not end. Stop, the way a man's breath stops at a tape.

"Nothing. Not nothing in him. Nothing in my hands." Oryn had not moved her hands.

She stood one stride inside a line in a rut on a road with a man's arm between her palms, and her face did what he had seen it do over his own ribs on a mountain, once, and never over anyone else's: the instrument returning nothing. Then she said, "Back," and Brom stepped back with her, and at the stride her eyes did the thing Lira's had done, and Karis's, and Brom's — went to a place and found it.

"There," she said. "What she said. The clock."

She did it three times.

Nobody told her to. She walked Brom over the stone and back, hands on the arm through all of it, the count running outside and stopping inside at the same stride, three times, because she was a professional and a thing that had happened once was a story, and at the third *back* she let go of his arm and stood at the stone on the near side with her hands at her sides, which Cael had not seen her do at this line, and said the sentence the carters had been saying to her for two years.

"A healer's hands stop working there."

She said it as a finding, to nobody, at the size Pike said things. Karis wrote it. Cael put it on the shelf beside Pike's *it stops*, because it was the same sentence with the hands in it.

---

"Cael," Karis said.

He had been at her shoulder with the book since the light came, and outside the stone all day, and she had noticed, because Karis noticed the one who had not been tried. "You've not been over."

"No."

"Twenty in. Tell me at the stride."

He put the book in the satchel and went to the stone.

The nine were held to nothing. They had been held to nothing since the east gate, and on the road last night, and he had counted them held while he counted cords, in the part of him that did that without being asked. He stood on the near side of the stone and, before the stride and not after it, took the hold off one thing.

The Iron-adjacent read. The cheapest thing he owned, and the quietest; a read of the ground through the soles, weight and give and what lay under the surface, taken from a man who always knew what a floor was doing under him. It wanted the road as it wanted every road. He gave it the road. It ran: the rut's packed edge under his left heel, the loose gravel at the rut's middle under the ball of the right, the stone's own weight where it sat, two fists, bedded a finger deep. Nothing anyone could see. A man standing on a road.

He stepped over.

It ran. The rut's far edge, packed like the near one; the grass past the rut, and the give of the root-mat under it; a pebble under his right sole the size of a thumbnail. He looked at the place, because Karis had said *tell me*, and the place was dark, as it had been dark on the road going in, as it had been dark for four years. The read went on running underneath the dark as if there were no dark. There was no dark. There was a read, and ground.

He walked in.

He counted the twenty in strides because he counted strides, through scrub to the knee along the line Seln had walked, with the read running under him the whole way — grass, stone, root, a soft place where the rubble had come down and settled — and at twenty he stopped, with his back to the stone and five people at it, and there was a fist of ridge-stone in the grass in front of his right boot.

He took the hold off a second thing, small.

The Wind-adjacent framework, a foot's worth. Not a burst. The other end of it: the evasion, the sidestep the framework took before the foot decided to, the thing he had learned on a training floor by watching a girl who did not know she was teaching it. A foot's worth of it, silent, round a stone in the grass.

It went. His right foot was a foot to the left of where it had been going, and the stone was beside it and not under it, and the framework had done it, not the foot; he knew the difference in his own legs as he knew his own name. From behind, from twenty strides, it was a man stepping round a stone.

He stood there for the length of a breath with his back to them. Nine fragments, held to nothing again, and two of them tried, and both of them ordinary, on ground where every Path at the stone behind him was dead at the stride.

Then he turned round and walked out, twenty strides, and stepped over the stone, and Karis had the pen ready.

"Well."

"It was dark before," he said. "It's dark now."

Both halves were true. Put together they were not the truth, and he knew the difference, and let them stand. Karis wrote them. Lira was looking at her own hands. Brom was looking at his arm. Oryn was looking at the stone. Nobody looked at him longer than it took to write a sentence, because everybody at the stone had a thing of their own that had just stopped, and he was the one who had said nothing had.

He catalogued that, and found he had a reason not to say more, and that it was the same two reasons as last night, and did not look for a third.

---

Six over the stone, then, with the sun in the west and the ring's floor beginning to take it, and the mule left on the road with two boxes and its rope on the root, because Karis had asked for the middle box and Brom had it by strap.

Karis walked it in cords. Ten cords from the stone across the scrub and the grass and over the lip, and the tenth cord's last knot lay on the floor at its middle, with the eleven standing round it. She did not say anything. She looked at the knot, and at the stone on the road behind them, and wrote *ten* in the book, and underlined it, and that was the finding said.

He catalogued the site, because he loved it and that was how.

Eleven uprights. He counted them a third time from inside, walking the ring, with a hand on each: eleven, standing at their own heights, the tallest to his shoulder and the shortest to his knee, none whole, each broken clean across at a different place as if a different thing had broken each. Grey. The same grey as the floor they stood on, and not the grey of the ridge's stone, and not the grey of anything Karis had a chip of.

The floor. He walked it edge to edge, twenty-six strides along the road's line, and twenty-six across it, and it was round to the eye. Level to the eye; and Karis ran her cord taut from one upright's foot to another's across the middle and lay down on the stone with her cheek on it and sighted along the cord, and moved the cord, and sighted again, and there was no daylight under it anywhere. Seamless. He went down on his knees at the middle, where the breath stopped at the tape, and put both palms flat, and let the read run on the stone under them, privately, a man touching a floor: it gave him one piece of stone from his hands to the lip, no join, no seam, no laid edge, nothing under the surface but more of the surface. Fused. Not laid. He did not write that word. He wrote *no seam* and let the read say the rest to him alone. At the edge, all the way round, the floor rose the height of a hand and turned over, as the rim of a shallow basin turns, so that a man standing on the grass outside it stood at a lip.

Karis had the middle box open on the floor by then, with the chips and the rubbings laid out in a row: Thornwater's mortar, the ridge's stone, and the masonries she had chips of, carried out of two academies and a records hall. She laid each against the floor's grey in turn, and looked, and put it back in the box. Then she said, "Not local. Not any masonry I've got a piece of," and shut the lid.

The lines. He had not seen them from the road and could not have; they were shallow, the depth of a fingernail, the width of a knife's back, cut round each upright in bands, regular as a rule, and they were in every band the same kind of line and never the same line. Karis had a sheet on the tallest upright with charcoal going across it before Cael had finished counting the bands on the second, and she copied through the afternoon without speaking, sheet after sheet, and when he asked her what alphabet, she said, "None I own. And I own more than the registry prints," and went on rubbing.

He walked the grass inside the lip, and the scrub out from it toward the road, and Seln walked the rest of the bowl, for the contract's instruments; and what there was on this ground that a person had put down and left was a bar of iron at the line with a number on it. Nothing on the floor. Nothing in the grass. Nothing in the scrub between the floor and the stone, or between the floor and any other part of the circle, and Seln had walked every part of it. A hundred marks on a slip in his satchel, and nothing to carry back to the plank for it but the number thirty-one.

"They're not here," Karis said, from the upright, without turning.

"No."

"Then the number's what we've got." She meant the stake's. She went on rubbing.

Oryn sat on the lip with her hands in her lap and her eyes on them, and did not raise them, and did not read anyone, because there was nothing here to read with. Lira, who had watched Karis work for a while, went and stood in the middle of the floor and set for a burst, and did the step, and stood on the spot with the breath coming out of her, and did it again, and then walked off the floor and sat down by Oryn and said nothing. Brom moved the box where Karis pointed. The floor took the sun. It went from grey to white along its whole width at once, as it had the evening before, and the eleven uprights stood on it with their bands of lines catching the light, and the light began to go long.

---

Seln, at the far lip, on one knee.

He had walked round the floor's edge on the outside of the lip while the rest were on the floor, slowly, looking at the ground the way he looked at a road, and at the far side, where the grass came up to the lip and the lip stood a forearm proud of it, he went down and stayed down, and did not call. Cael saw him stay down and went; and then Brom; and then the rest.

Tracks, first. Many. The round grey-dust print he had seen in the ford's mud with his own blood on it, here in a bare patch of earth at the lip's foot, over and over, pressed into each other so that the earth was print and nothing else. Bones, past them, in the grass: long ones, cracked for the marrow, and a jaw, and something with the hair still on it. Scat, heavy, the dark of a day old and no more. And under the lip, where the fused stone stood out from the earth with nothing beneath it but earth, the earth had been dug back, and back, a hollow going in under the floor further than the light reached, its mouth worn as smooth as a doorstep by things going in and out of it low.

Seln had a thumb in one of the prints, testing the dust's edge.

"Fresh," he said. "Today's. A lot of them." He looked into the hollow, at the dark under the stone they had spent the afternoon standing on. "They live in here because nothing that walks in can fight."

Nobody answered him.

Cael watched them look, because he was the one who watched. Lira looked at the line: two hundred meters back, west, through scrub, where a two-fist stone sat in a rut with a mule beside it. Brom looked at the line. Karis, with charcoal on both hands, looked at the line. Seln, from his knee, looked at the line, and then at the sun, which was on the ridge above the road and going, and then at the line again, measuring it. Oryn looked at the line with her hands at her sides. Then every one of them, one after another, looked at Oryn, whose hands did not work in here, and Oryn looked at the sun.

And then, one after another, they looked at him.

He was not looking at the line. He had looked at it once, when Seln said *today's*, and it had been a stone in a rut two hundred meters away, and he had known what it was to every one of them and known that it was nothing to him, and had not needed to look at it again. He was looking at the mouth of the den, and at Seln's thumb in the print, and at the light going off the floor behind him, and when he felt the five of them look at him he took the book out and wrote, standing, in the plain hand, while Brom got the box onto his back and Seln stood up off his knee.

*Ninety-sixth day. The Fallow Ring, inside the line, at the far lip. A hundred and eight in hand; nothing owed. Eleven uprights. One stake. Eighteen.*

*The circle is perfect. The stakes are the registry's. The dogs are inside. Everyone's Path dies at the line and mine didn't, and I've told nobody, and I'm going to need to think about why I haven't. Not tonight. Tonight we're inside the one place on the continent where a stillhound can't lose.*
...
# MANUSCRIPT — v3-runs/book-07/drafts/ch11.md
...
# AUTHOR REPORT — v3-runs/book-07/reports/ch11-author-r1.json

{
  ...
  "obligations": [
    {
      "obligation": "must_include: Five exchanges as staged, with the boundary as terrain: the lip, the den side, the lane, the line as objective; geography drawable at every pause; at least two beats caused by terrain (the lip of fused stone Brom holds; the line itself; the one meter inside the edge where Karis lights).",
      "status": "MET",
      "evidence": "The stone was ten cords west of the floor's middle, along the straight line Karis had laid her cord down that afternoon; the road's rut ran a stone's throw north of that line and met it at the stone. | \"The road's cut.\" | Brom held the lip. | the rim was a hand's height and turned over like a basin's, and its forefeet came down on grass a hand lower than its hind feet expected, and for one beat it had no drive in it at all. | Her right heel came down on the rim's turned edge. | A hundred and sixty meters. | Sixty meters. | He crossed the line at a run. | along the line's inside edge, the meter of ground between the scrub and the stone that every hound in the circle had crossed every night without once needing to be careful of it."
    },
    {
      "obligation": "must_include: Stillhounds hunt by declaration and by sound; silent bodies nearly invisible to them; the unit quiet by training; the pack has never met prey inside that could be loud.",
      "status": "MET",
      "evidence": "a still body was nearly nothing to them, and Brom, who at the ford had learned in one contact that Iron Skin was a shout, was the stillest large thing on the ring. | They hunted by sound now, and every head that came to the rim came to a noise, and the six of them had been taught for four years of their lives to make none. | Prey that walked into this circle could not shout, and had never once been able to, and there was not an animal in this scrub that had ever needed to learn what a shout was."
    },
    {
      "obligation": "must_include: Lira's third ankle turn at the lip; the forearm bite; Brom's four-meter bolt; Oryn's failed reading inside with her two-sentence order verbatim; the binding with cloth and knees.",
      "status": "MET",
      "evidence": "He heard it go, the third time, and the sound was the one it had made on a bar. | It took it below the elbow with its whole jaw and did not let go. | she could not burst it off, because a burst was a shout and there was no shout in her here | Brom's bolt went through its eye at four meters. | \"Nothing,\" Oryn said, to the arm. Her hands were to the wrist in what was coming out of it. \"Nothing in my hands.\" | got her knee up onto Lira's upper arm above the elbow, all her weight on it | Cloth and knees. The bleeding slowed under the cloth. It did not stop. | \"Move,\" she said. \"I can't fix this here. Move.\""
    },
    {
      "obligation": "must_include: Karis down under a hound at sixty meters; Cael's Wind-adjacent declaration and the run PAST the pack to the line; the pack following the declaration out; five Paths returning at the stride; Karis's ignition-wall on the inside edge; the break; two dead, seven gone.",
      "status": "MET",
      "evidence": "Sixty meters.\n\nKaris went down. | He saw her hands go to its throat. | Wind-adjacent, full. | the loudest thing that had ever been inside this circle, on purpose, west, past the pack, along the lane toward a stone in a rut. | Every head turned. | The pack came over the stone after the declaration. | heard Seln say \"Back,\" to nobody, in the voice he used for a distance, and heard Lira, from the ground, say it a breath after him. | \"There,\" she said. \"Back.\" | a big man who had just declared — Iron Skin, once, and off again — and a girl on the road who had just declared, Wind | She lit it.\n\nA wall of ignition stood up across the line, twenty paces of it, thirty, white, at the height of a hound's head | They broke.\n\nNot back. | Two on the ground. One at the lip, inside, with a bolt through its eye; one in the rut against the stone, with a bolt through its chest, its forefeet across the line and its hind feet inside it. Seven gone."
    },
    {
      "obligation": "must_include: Costs outside the line as listed; Oryn's Path returning at the stride and the hour's mending at the line by lamplight; the arm's state DEFINED on the page (saved; vessels and deep tear closed; bears no load for a season; Oryn's order: not to hold, strike, or catch until she says; one-armed to first snow).",
      "status": "MET",
      "evidence": "She had gone to it the stride she was over the stone, and Cael, sitting in the rut with the nine gone to nothing in him — not held to nothing; gone — watched her face do what it had done at the stone that morning, going the other way. The count started. | \"Lamp,\" she said, to Brom, who had the mule's panniers open before the word was finished and the carter's lantern out of them and lit | The hour was an hour. Karis timed it on the ridge's dark, and wrote the time down. | \"The vessels are closed. Both. The deep tear's closed; muscle to bone. The bone's marked and it'll stay marked and it's whole.\" She looked at the arm, and at Lira, and at nobody else. \"She keeps it.\" | \"A mend that deep holds nothing for a season. Nothing. That arm is not to hold. It's not to strike. It's not to catch. Not a rope, not a knife, not a fall, not a friend. Not until I say. And I'll say at first snow, near enough, if it's let alone.\" | \"One arm. From here to the snow.\" | her throat under the jaw was going dark on both sides in the shape of a mouth, and when she had tried to say a word at the stone what had come out was breath. | with his right hand open on his knee and the fingers of it laid open across the pads by the string | said, \"Nothing,\" which from Seln was a whole report | Oryn lay down on her back on the road with her hands on her stomach, and did not get up for a while, and nobody asked her to."
    },
    {
      "obligation": "must_include: Cael's deployment inventory honest: Wind-adjacent full; Shadow-adjacent once inside (witnessed by circle and Oryn only); other fragments as the run requires; spent to the ground; first full-suite use since the Daeva match; calf reopened.",
      "status": "MET",
      "evidence": "Shadow-adjacent. Once. | there was no public here — four people who knew where it came from, a healer who had found nothing in him three times, and eight animals. | Pressure-adjacent, because it was loud and he needed it for nothing else. Ember-adjacent, one point of ignition held out at arm's length as he ran, Karis's | Compression-adjacent, Reydan's, at contact | his own right calf caught the rim's turned edge as he turned them off it, and went hot along the old line where the ford had gone through, in and out. He filed it as reopened | Spent. Not tired; spent — the frameworks gone to nothing with a hollow in them where each one had rung, all nine, one after the other, as they had not gone since a matting floor against a woman who was better than him. | *Inventory. Nine confirmed, one anomaly. Deployment inside the line, all witnessed by the circle and by Oryn and by no one else on the continent: Shadow-adjacent, once, ten strides, to reach Karis. Then everything. Wind full, two bursts, two locks, a dog in the second. Iron the whole way. Pressure. Ember, one point, running. Blade. Storm. Compression at contact. Anchor, perceiving. All nine. First time I've spent the whole suite since Daeva. Bill: spent to the road; ribs at the tape; right calf open again on the lip, the ford's line; two landing beats.*"
    },
    {
      "obligation": "must_include: Everyone saw; nobody says anything; the priority was Lira's arm.",
      "status": "MET",
      "evidence": "Five people had watched him do it. Four practitioners with no Paths and a healer with no hands had watched the sixth one of them use his, inside the circle, all of it, and lead the pack out across a line with it — Brom from the lip, Seln from the lane, Karis from the ground with a dog on her, Oryn with Lira's arm in her hands, Lira from his shoulder — and not one of them had said a word about it, and not one of them was going to tonight, because the priority on this road was under a lantern with Oryn's hands on it, and everybody knew it. | *Five people saw all of it. Nobody's said a word. Lira's arm came first. It'll come first tomorrow.*"
    },
    {
      "obligation": "must_include: The chapter ENDS on the Lira/Cael exchange verbatim.",
      "status": "MET",
      "evidence": "\"It worked in there.\"\n\nA beat.\n\n\"Yours.\"\n\n\"Yes,\" Cael said.\n\n\"Later.\"\n\n\"Later.\"\n"
    },
    {
      "obligation": "must_include: Manuscript format: H1 '# Chapter 11 — The Quiet', prose only, '---' breaks, ends on the last line of prose.",
      "status": "MET",
      "evidence": "# Chapter 11 — The Quiet\n | \n---\n"
    },
    {
      "obligation": "plant: Cael's fragments work inside the Quiet — now SEEN by five people (Ch12 silence; Ch16 disclosure).",
      "status": "MET",
      "evidence": "Four practitioners with no Paths and a healer with no hands had watched the sixth one of them use his, inside the circle, all of it | Nothing in him changed, and he catalogued it at the run: the stride over the stone, going out, and the nine ringing in him exactly as they had rung a stride inside, and the place where an Arbiter would have been as dark on the road as it had been on the floor."
    },
    {
      "obligation": "plant: Lira one-armed for the season (Ch14–24).",
      "status": "MET",
      "evidence": "\"One arm. From here to the snow.\" | *Vessels closed. Deep tear closed. Bone marked, whole. Saved. No load for a season — not to hold, strike, or catch until she says. First snow. One-armed.*"
    },
    {
      "obligation": "plant: Oryn's Path returns at the stride (Ch19, Ch21).",
      "status": "MET",
      "evidence": "It had stopped inside, and here on the road with the same arm between the same palms it went from where it had stopped to ten, and her eyes went to a place and found it there.\n\n\"There,\" she said. \"Back.\""
    },
    {
      "obligation": "plant: 'Later.' — Lira first (Ch16).",
      "status": "MET",
      "evidence": "\"Later.\"\n\n\"Later.\"\n"
    },
    {
      "obligation": "payoff: Ch3's bait-with-one-declaration, inverted.",
      "status": "MET",
      "evidence": "He knew what they followed. He had learned it on a river from the thing that did not happen: the loudest declaration, and absent one, the loudest sound, and absent that, whichever thing on the floor committed first. They had never met a thing on this floor that could declare. | *One sentence, because the ledger wants one: they live in there because nothing that walks in can fight, and I walked in, and I could, and I took their floor away by being the one thing on it they could follow out. The ford turned over: the declaration ran, and the killing was a line.*"
    },
    {
      "obligation": "payoff: Ch6's ignition-wall.",
      "status": "MET",
      "evidence": "hers were laid along the ground in a row so close they were one thing, as she had laid them across a mountain"
    },
    {
      "obligation": "payoff: Ch10's den, sun, and Oryn's hands.",
      "status": "MET",
      "evidence": "walking the last five strides backward.\n\nThe floor had gone white along its whole width at once, as it had the evening before, and it stayed white while the grass round it went grey. | Both hands. The count that went to ten, and he watched her face do what it had done over Brom's arm at the stone that morning: the count stop at the stride."
    }
  ],
  "inventions": [
    "The carter's crossbow is given a provenance: it is Oryn's, carried two years under her mule's panniers because carters carry crossbows through quiet ground (reference section), handed to Brom at the stone during the Chapter 10 afternoon crossing (which Chapter 10's page does not itemize), with six bolts in a leather sleeve on his belt beside his Chapter 3 bill-hook. Oryn states she has never wound it at anything and does not mean to start — she is not made to fight.",
    "Brom holds the west lip with the bill-hook's haft and his body rather than shooting: a hound coming over the hand-high, turned-over rim lands with its forefeet a hand lower than its hind feet and has one beat with no drive; Brom puts his weight through the haft into its neck and sends it back onto the floor (terrain beat one).",
    "The cut and the lane made drawable: the cut-dog sits in the road's rut forty meters out from the floor; the uncut lane is Karis's cord-line (grass and low scrub) south of the rut and parallel to it, meeting the road at the stone; the rut runs a stone's throw north of the cord-line.",
    "Lira's bitten forearm is the LEFT (the knife stays in her right); she stabs the hound behind the ear twice and it does not let go; the dead hound lies across her legs with the arm still in its jaws and Cael opens the dead jaws with his knife's back and his fingers.",
    "Oryn's binding staged in detail: knee on the upper arm above the elbow with her whole weight, a wad over the tear, two strips wound and twisted, a third above, tighter — 'without a Path in any of it'.",
    "Cael's calf reopens on the rim's turned edge as he lifts Lira off the lip (the ford's entry/exit line), filed as reopened.",
    "Brom's right hand is cut when he spans the crossbow by hand in the dark after the eye-shot: the string gets out from under his fingers and lays them open across the pads; he wraps it himself later with Oryn's linen, Cael holding the end; Oryn declines to touch it until morning.",
    "Cael carries the middle box along the lane (Brom needs both hands; Karis needs the box) and drops it at Karis's fall; Seln goes back sixty meters into the dark for it while the wall is still burning and returns with it, saying nothing.",
    "Karis is taken at the hip by a hound out of the north scrub as she comes back to take Lira's other side; the hound's jaws close on her throat across the backs of her own fists ('as far as her fists let them, which was far enough') — bruising, not puncture; Cael hauls the hound off by scruff and hind leg and throws it into the south scrub; it is not killed (dead count stays two).",
    "The Shadow-adjacent use is rendered as observation only: he thins, crosses ten strides through a pack turning to Karis's voice, and no head turns to him — 'If it counted as a declaration, nothing on the floor came to it. He did not have a sentence for that either. He put it on the shelf.' No theory.",
    "The run to the line is a chain of declarations so that 'there was never a breath in which he was not the loudest thing on the floor': Wind (two bursts, two landing locks), Pressure, Ember (one point held out as he runs), Blade, Storm (corridor seeded ahead along the lane), Anchor (perception only — he sees Karis's lattice laid before it lights), Iron read running under his soles throughout, Compression at contact when a hound reaches him in the second lock and goes over him. No fragment but Wind resolves anything; the others are noise and cost.",
    "The pack's break staged so that the lattice-line goes up on the line's inside edge BEHIND the seven that are across: the floor and den are on the far side of the fire, in front of them a big man declaring Iron Skin once, Lira declaring Wind from the ground, and Seln gone from beside the stone; they break north and south along the line's outside through the scrub. The first hound over the stone takes Brom's bolt through the chest at two meters and dies in the rut with its forefeet across the line.",
    "Oryn's clinical findings before and after the mend: 'The vessels are open. Two of them. The muscle's torn through to the bone on the inside, and the bone's marked.' / 'The vessels are closed. Both. The deep tear's closed; muscle to bone. The bone's marked and it'll stay marked and it's whole.' She calls it the worst she has had her hands in for two years and forbids talk during the hour; Karis times the hour and writes it down.",
    "Karis, voiceless, writes the arm's state and her own throat note in the notebook and shows Cael; she then writes something else and does not show him.",
    "A short Power Log entry mid-aftermath (not the chapter's ending), in the established plain hand, carrying the honest deployment inventory, the six injuries, the two-dead/seven-open count, the one-sentence win, and the plant ledger's Chapter 11 Arbiter line: 'Everyone else's went to sleep at the stone. Mine has been asleep since I was fourteen. Out here that makes me the only one who's awake.'",
    "The carter's lantern comes out of the mule's panniers for the mending; the retreat is counted in waypoints (a hundred and sixty, a hundred and forty, a hundred and twenty, a hundred, eighty, sixty, twenty meters) in Cael's cataloguing register."
  ],
  ...
  "deviations": [
    "Length: the first pass overshot (6,445 words by wc -w) and was compressed in two passes to 5,194 — inside the packet's tolerance (4,600 ±15%, ceiling 5,290), in the upper part of the band. The packet asked for draft-to-length rather than overshoot-and-trim; reported honestly. All obligations survived the compression; nothing was summarized away.",
    "Crossbow provenance (continuity seam): no crossbow is on the unit anywhere in Chapters 1–10 (the unit carries knives and Brom's bill-hook; carters carry crossbows). The card and packet mandate 'a carter's crossbow' and Brom's bolts, so the draft makes it Oryn's (a carter's crossbow carried two years, per the reference section's etiquette) and has her hand it to Brom at the stone during the Chapter 10 afternoon crossing — a moment Chapter 10's page does not itemize. Chapter 10 is not modified. Editor may accept the retroactive detail or prefer a different provenance; a one-line seam note would resolve it either way.",
    "Opening distance: the packet's scene_shape says '~140 m inside'; the draft opens the fight at the ring's west lip (about a hundred and ninety meters from the stone by Chapter 10's ten-cord radius) so that the mandated lip exchanges (Brom holding the lip; Lira running it) are physically AT the lip, and uses 'a hundred and forty' as a retreat waypoint. Karis goes down at sixty meters as mandated. All distances stated are consistent with Chapter 10's measurements.",
    "Single-word-paragraph cap (charter §9: 2 per chapter): the draft has three, all of them the packet's verbatim closing exchange ('Yours.' / 'Later.' / 'Later.'); no narrative single-word paragraphs. Reported as the mandated exception.",
    "Karis's ignition-wall 'on the inside edge of the line' (mandated): staged from her knees a stride outside the stone, the points laid 'along the line's inside edge'; Cael, perceiving the lattice with Anchor-adjacent, 'could not afterward have said' whether the points lay a hand inside or outside the line. Chapter 10's trials established only that a practitioner standing INSIDE cannot ignite; nothing on the page theorizes why a lattice laid from outside along the edge lights. Flag for the editor's mechanism gate; the packet requires the beat.",
    "Shadow-adjacent inside: rendered so that no head turns to him while the pack converges on Karis's voice, with the narration limited to 'If it counted as a declaration, nothing on the floor came to it' — an observation Cael shelves, not a theory. The packet permits this use; flag for the 'no unexplained effect on the pack' gate that the hounds' non-reaction to Shadow is shown, not explained.",
    "'Full suite': all nine fragments are spent in the run, but Anchor-adjacent is used as perception only (he sees Karis's lattice laid before it lights); no fixed point is set. Reported so the editor can decide whether perception counts toward 'full-suite'.",
    "A mid-chapter Power Log entry is included (not mandated by the packet; not the ending — the chapter ends on the mandated dialogue). It carries the honest inventory and the architecture plant ledger's Chapter 11 line for the Book 9 plant ('Everyone else's went to sleep at the stone. Mine has been asleep since I was fourteen. Out here that makes me the only one who's awake.'), which the packet's plants list does not name but the arc document assigns to this chapter. The log also writes the seven fled hounds as an open finding.",
    "Lira's Wind and Brom's Iron Skin are each declared ONCE outside the line at the stone as the pack breaks (the card's 'five practitioners have their Paths back at the stride' rendered as Seln's and Lira's 'Back', Oryn's count, Brom's Iron Skin up and off, Lira's burst from the ground); no one but Cael declares inside. The declarations outside are presented as part of why the pack breaks outward; the kills remain Brom's bolts only.",
    "Brom fires exactly two bolts, both kills ('Two down to Brom's bolts'); he does not shoot at the lip's first exchanges, holding it with the hook's haft and his body instead, so that the dead count stays at two. No bolt is retrieved on the page.",
    "Seln's retrieval of the middle box from sixty meters inside during the wall is an invented texture beat (the box had to leave Brom's back for him to shoot and carry Karis); logged so the box's whereabouts at Chapter 12 are defined (recovered, at the stone).",
    "Karis's throat injury is kept to bruising (the hound's jaws close on her throat across the backs of her own fists) — no puncture, no bleeding — to stay inside the packet's injury list (throat bruised, voice gone)."
  ],
  "blockers": []
}
