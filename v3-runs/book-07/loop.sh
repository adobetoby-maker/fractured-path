...
case $cmd in
  ...
  run-editor)  # validate author report, compile editor prompt, launch codex (Sol) detached
    ...
    nohup codex exec --cd $ROOT --skip-git-repo-check -o $B/reports/ch$N-editor-r$R.lastmsg.md "$(cat $B/packets/ch$N-editor-wrapper-r$R.md)" < /dev/null > $B/reports/ch$N-editor-r$R.stdout.log 2>&1 &
