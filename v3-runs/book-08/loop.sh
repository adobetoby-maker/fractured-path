...
set -e
ROOT=/Users/drive/fractured-path; P=/Users/drive/penname/pennamecodexv3; B=$ROOT/v3-runs/book-08
cmd=$1; N=$2; R=${3:-1}
case $cmd in
  ...
  run-editor)  # validate author report, compile editor prompt, launch codex (Sol) detached
    ...
    sed "s|__PROMPT__|v3-runs/book-08/packets/ch$N-editor-prompt-r$R.md|g; s|__OUT__|v3-runs/book-08/reports/ch$N-editor-r$R.json|g; s|__PREV__|$( [ "$R" != "1" ] && echo "Your prior report on this scene is at v3-runs/book-08/reports/ch$N-editor-r$((R-1)).json and the verifier decisions at v3-runs/book-08/reports/ch$N-verifier-r$((R-1)).json; the pre-repair draft is in CONTEXT. Confirm each verified finding is repaired (quote), confirm no unrelated passage changed and every recorded strength survived, then re-run the full gate set." || echo "This is a first review of a fresh draft." )|" $B/editor-wrapper.template.md > $B/packets/ch$N-editor-wrapper-r$R.md
...
PY
  ...
  close)  # splice and record
    cp $B/drafts/ch$N.md $ROOT/books/book-08-before-the-paths/chapters/chapter-$N.md && echo "spliced chapter-$N.md ($(wc -w < $B/drafts/ch$N.md) words)";;
esac
